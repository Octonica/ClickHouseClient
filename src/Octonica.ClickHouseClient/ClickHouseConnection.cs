                ...
                switch (message.MessageCode)
                {
                    case ServerMessageCode.Hello:
                        ...
                        if (serverInfo.Revision >= ClickHouseProtocolRevisions.MinRevisionWithAddendum)
                        {
                            ...
                            {
                                // TODO: chunked mode support
                                if (helloMessage.SendChunkedMode == ChunkedPacketsMode.Chunked || helloMessage.ReceiveChunkedMode == ChunkedPacketsMode.Chunked)
                                    throw new ClickHouseException(ClickHouseErrorCodes.FeatureNotImplemented, "Chunked packets mode required by the server is not supported.");
                            }

                            var clientHelloAddendum = clientHelloAddendumBuilder.Build();
                            clientHelloAddendum.Write(writer);
                            writer.Write7BitInt32((int)ClientMessageCode.Ping);
                            await writer.Flush(async, cancellationToken);

                            var extraMessage = await reader.ReadMessage(clientHelloAddendum.ProtocolRevision, false, async, cancellationToken);
