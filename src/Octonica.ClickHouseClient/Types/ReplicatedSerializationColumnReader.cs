...
internal sealed class ReplicatedSerializationColumnReader : IClickHouseColumnReader
{
    ...
    public ReplicatedSerializationColumnReader(IClickHouseColumnTypeInfo columnType, int rowCount, LowCardinalityTableColumnNullableMode nullableMode)
    {
    ...
    public SequenceSize ReadNext(ReadOnlySequence<byte> sequence)
    {
        ...
        if (_indices == null)
        {
            ...
            ++totalBytes;
            _indices = new byte[checked(_keySize * _rowCount)];
        }

        seq = sequence.Slice(totalBytes);
        var len = Math.Min(_indices.Length - _indicesOffset, checked((int)seq.Length));

        seq.Slice(0, len).CopyTo(_indices.AsSpan().Slice(_indicesOffset, len));
        _indicesOffset += len;
        totalBytes += len;

        if (_indicesOffset < _indices.Length)
        ...
        _baseReader = _columnType.CreateColumnReader(_baseRowCount);
    ...
    public IClickHouseTableColumn EndRead(ClickHouseColumnSettings? settings)
    {
        if (_baseReader == null)
        {
            var emptyBaseReader = _columnType.CreateColumnReader(0);
            return emptyBaseReader.EndRead(settings);
        }

        var baseColumn = _baseReader.EndRead(settings);
        return new LowCardinalityTableColumn(new ReadOnlyMemory<byte>(_indices), _keySize, baseColumn, _nullableMode);
    }
}
