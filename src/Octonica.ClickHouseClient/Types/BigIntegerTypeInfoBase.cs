...
namespace Octonica.ClickHouseClient.Types
{
    internal abstract class BigIntegerTypeInfoBase : SimpleTypeInfo
    {
        ...
        public sealed override IClickHouseColumnWriter CreateColumnWriter<T>(string columnName, IReadOnlyList<T> rows, ClickHouseColumnSettings? columnSettings)
        {
            ...
            return bigIntegerRows == null
                ? throw new ClickHouseException(ClickHouseErrorCodes.TypeNotSupported, $"The type \"{type}\" can't be converted to the ClickHouse type \"{ComplexTypeName}\".")
                : (IClickHouseColumnWriter)new BigIntegerColumnWriter(columnName, ComplexTypeName, _elementByteSize, bigIntegerRows, _isUnsigned);
        }
        ...
        public override IClickHouseParameterWriter<T> CreateParameterWriter<T>()
        {
            ...
            {
                writer = _isUnsigned
                    ? new StringParameterWriter<BigInteger>(
                        this,
                        bigIntegerValue =>
                        {
                            return bigIntegerValue.Sign < 0
                                ...
                                : bigIntegerValue.ToString(CultureInfo.InvariantCulture).AsMemory();
                        })
                    : StringParameterWriter.Create<BigInteger>(this);
            }
            else if (type == typeof(ulong))
            {
            ...
            return writer == null
                ? throw new ClickHouseException(ClickHouseErrorCodes.TypeNotSupported, $"The type \"{type}\" can't be converted to the ClickHouse type \"{ComplexTypeName}\".")
                : (IClickHouseParameterWriter<T>)writer;
        }
        ...
        private sealed class BigIntegerColumnReader : IClickHouseColumnReader
        {
            ...
            {
                _buffer = rowCount == 0 ? Array.Empty<byte>() : (new byte[rowCount * elementByteSize]);
            ...
            {
                int rowCount = _buffer.Length / _elementByteSize;
                if (_position >= rowCount)
                {
                    throw new ClickHouseException(ClickHouseErrorCodes.DataReaderError, "Internal error. Attempt to read after the end of the column.");
                }

                int elementCount = (int)Math.Min(rowCount - _position, sequence.Length / _elementByteSize);
                int byteCount = elementCount * _elementByteSize;
        ...
        private sealed class BigIntegerColumnWriter : StructureWriterBase<BigInteger>
        {
            ...
            protected override void WriteElement(Span<byte> writeTo, in BigInteger value)
            {
                ...
                int byteCount = value.GetByteCount(_isUnsigned);
                ...
                bool success = value.TryWriteBytes(writeTo[..byteCount], out int bytesWritten, _isUnsigned);
                Debug.Assert(success);
                Debug.Assert(byteCount == bytesWritten);

                if (byteCount < ElementSize)
                {
                    writeTo[byteCount..ElementSize].Fill(_isUnsigned || value.Sign >= 0 ? (byte)0 : (byte)0xff);
                }
            }
        }
    }
